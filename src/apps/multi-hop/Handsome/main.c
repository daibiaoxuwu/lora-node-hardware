...
int isRouterOrData(void)  //0 neither 1 router 2 data
{
    ...
    if(mhdr.Bits.Major == 0 && mhdr.Bits.MType == 7 && mhdr.Bits.RFU == 1)
		{   //packets of mesh lora				
				...
				else if(Buffer[1] == 10 && Buffer[7] == ((uint16_t)DEVICE_ADDRESS & 0xFF) 
				&& Buffer[8] == (((uint16_t)DEVICE_ADDRESS>>8) & 0xFF)){
						return 2;
				}
		}
...
{  
	  if(pt == RTS){
			  ...
			  uint8_t AppData[MESHLORA_APPDATA_PAYLOAD_LENGTH];
				...
				//add to save buffer
				if((bf_save_data_have + 1) % BUFFER_SAVE_DATA == bf_save_data_now){//full
					lost_data += 1;
					printf("lst data: %d\n", lost_data);
				}
				else{
...
void OnSendRouterTimerEvent(void)
{
    ...
    TimerStop(&SendRouterTimer);
	  ...
	  if(meshLoRaRouterSequenceNum == 10){
...
{   //only relayData, sendData, sendRouter calls
	  printf("CAD Done\n");
	  //printf("(rd)%d\n", Radio.GetStatus());
        ...
        switch(State){
            case RX:
                if(BufferSize == RTS0_ACKS_SIZE && isMeshLoRaPkts()){
                    ...
                    else if(Buffer[1] == 2){//ack0
											  if(Buffer[2] == ((uint16_t)DEVICE_ADDRESS & 0xFF) 
                           && (Buffer[3] == (((uint16_t)DEVICE_ADDRESS >> 8) & 0xFF))){
                         			printf("(%d)rx ack0\n", RtcGetTimerValue());
                              getAck = true;
                              sendRouter();											 
										    }
													  ...
													  printf("back-off\n");
													  TimerStop(&BackOffTimer);
													  TimerSetValue(&BackOffTimer, BACKOFFTIME + randr(-50, 50));
													  TimerStart(&BackOffTimer);
												}
                    }
                    else if(Buffer[1] == 3){//ack1
											  if(Buffer[2] == ((uint16_t)DEVICE_ADDRESS & 0xFF) 
                           && (Buffer[3] == (((uint16_t)DEVICE_ADDRESS >> 8) & 0xFF))){
												    printf("(%d)rx ack1\n", RtcGetTimerValue());
														getAck = true;
                ...
                else if(BufferSize == RTS1_SIZE && isMeshLoRaPkts()
									  && Buffer[1] == 1 && (Buffer[2] == ((uint16_t)DEVICE_ADDRESS & 0xFF)) 
                    && (Buffer[3] == (((uint16_t)DEVICE_ADDRESS >> 8) & 0xFF))){//rts1
									  if(sendingACKAndWaitData == 1){
												//must wait for data at least 2 times
											  if(misDataCount == 2){
													  misDataCount = 0;
									  ...
									  printf("rx rts1\n");
										if(bf_save_relay_now == (bf_save_relay_have + 1) % BUFFER_SAVE_RELAY){  //already have frames to relay
											  printf("fulRe\n");
											  relayData();
										}
										else{
                    ...
                    if(pkt == 1){//router
                        ...
                        else
                        {
													  ...
													  if(addr > RELAY_NODES){
															  ...
															  if((bf_save_relay_have + 1) % BUFFER_SAVE_RELAY == bf_save_relay_now){//full
																	lost_relay += 1;
																	printf("lst relay: %d\n", lost_relay);
																}
																else{
            ...
            case TX:
                if(Ptype == RTS){
                    printf("(%d)send RTS\n", RtcGetTimerValue());
                    Radio.Rx(RTSINTERVAL + randr(-100, 100));
									  //printf("(rd)%d\n", Radio.GetStatus());
                }
                else if(Ptype == ROUTER){
                    printf("send Router, len %d, cnt %d\n", BufferSize_send, meshLoRaRouterSequenceNum);
                    ...
                    if(sendingACKAndWaitData == 1){
                        ...
                        Radio.Rx(WAITFORDATATIME);
									  ...
									  printf("send Data, len %d, cnt %d\n", Buffer_save_data_len[bf_save_data_now], meshLoRaDataSequenceNum);
                    getAck = false;
									  /*
